...
class Word2VecAnalyzer(Analyzer):
    ...
    def evaluate_model(self) -> None:
        ...
        w2v_model: word2vec.Word2Vec = word2vec.Word2Vec(name=self.model_name)
        search_query: Dict[str, List] = self.search_query
    ...
    def evaluate_batch_for_outliers(self, w2v_model: word2vec.Word2Vec, eval_sentences: List,
        ...
        for i, single_sentence_prob in enumerate(sentence_probs):
            ...
            decision_frontier: Union[int, float, np.float64] = helpers.utils.get_decision_frontier(
                self.model_settings["trigger_method"], unique_probs, self.model_settings["trigger_sensitivity"],
                self.model_settings["trigger_on"])
            is_outlier: Union[int, float, np.float64, bool] = helpers.utils.is_outlier(
                single_sentence_prob, decision_frontier, self.model_settings["trigger_on"])
            if is_outlier:
                fields: Dict = es.extract_fields_from_document(
